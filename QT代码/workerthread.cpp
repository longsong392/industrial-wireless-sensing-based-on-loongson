...
void WorkerThread::process()
{
    ...
    emit uploadStatus("ESP8266初始化完成");
    emit uploadStatus("所以设备将初始化完成");
...
void WorkerThread::handleLoraData(const Lora::LoraData &data)
{
    ...
    if (esp && esp->isConnected()) {
        ...
        if (data.nodeId == 2) {
            bool success = esp->upload_data_node2(product_id, device_name, data);
            emit uploadStatus(success ? "上传成功" : "上传失败");}
        if (data.nodeId == 3) {
            bool success = esp->upload_data_node3(product_id, device_name, data);
            emit uploadStatus(success ? "上传成功" : "上传失败");}
    }
}
