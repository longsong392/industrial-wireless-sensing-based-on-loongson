    ...
    , currentSelectedNode(-1)
{
    ...
    for (int i = 0; i < 10; ++i) {
        pointTimes[i] = initialTime.addSecs(i * timeInterval * timeScaleFactor);
    }
    ...
    workerThread->setPortNames("/dev/ttyS1", "/dev/ttyS2");
...
void MainWindow::updateTimeDisplay()
{
    ...
    int realSeconds = initialTime.secsTo(currentTime);
    int scaledSeconds = static_cast<int>(realSeconds * timeScaleFactor);

    const int timeInterval = 10; // 时间间隔（秒）
    for (int i = 0; i < 10; ++i) {
        pointTimes[i] = initialTime.addSecs(scaledSeconds + i * timeInterval * timeScaleFactor);
    }
...
void MainWindow::onListWidgetItemClicked(QListWidgetItem *item)
{
    ...
    updateAlarmButtonTexts();
}
void MainWindow::updateAlarmButtonTexts()
{
    if (currentSelectedNode >= 0 && currentSelectedNode < 3) {
        QString nodeName = QString("节点%1").arg(currentSelectedNode + 1);

        ui->pushButton_10->setText(nodeName + "烟雾报警");
        ui->pushButton_11->setText(nodeName + "温度报警");
        ui->pushButton_12->setText(nodeName + "一键报警");
        ui->pushButton_13->setText(nodeName + "一键停止");
    } else {
        // 未选择节点时的默认文本
        ui->pushButton_10->setText("烟雾报警");
        ui->pushButton_11->setText("温度报警");
        ui->pushButton_12->setText("一键报警");
        ui->pushButton_13->setText("一键停止");
    }
}
...
void MainWindow::Smoke_Warning() {
    if (currentSelectedNode >= 0) {
        QString message = QString("%1mbj").arg(currentSelectedNode + 1);
...
void MainWindow::Temp_Warning() {
    if (currentSelectedNode >= 0) {
        QString message = QString("%1tbj").arg(currentSelectedNode + 1);
        QMetaObject::invokeMethod(workerThread, "slotWriteToLora", Qt::QueuedConnection,
                                  Q_ARG(QString, message));
    }
}
...
void MainWindow::OneTap_Warning() {
    if (currentSelectedNode >= 0) {
        QString message = QString("%1jbj").arg(currentSelectedNode + 1);
        QMetaObject::invokeMethod(workerThread, "slotWriteToLora", Qt::QueuedConnection,
                                  Q_ARG(QString, message));
    }
}
...
void MainWindow::Stop_Warning() {
    if (currentSelectedNode >= 0) {
        QString message = QString("%1nbj").arg(currentSelectedNode + 1);
        QMetaObject::invokeMethod(workerThread, "slotWriteToLora", Qt::QueuedConnection,
                                  Q_ARG(QString, message));
    }
}
...
void MainWindow::Send_Light_Value() {
    ...
    if (currentSelectedNode >= 0) {
        QString message = QString("%1L%2").arg(currentSelectedNode + 1).arg(light);
        QMetaObject::invokeMethod(workerThread, "slotWriteToLora", Qt::QueuedConnection,
                                  Q_ARG(QString, message));
    }
}
void MainWindow::Send_Temp_Value() {
    ...
    if (currentSelectedNode >= 0) {
        QString message = QString("%1T%2").arg(currentSelectedNode + 1).arg(temp);
        QMetaObject::invokeMethod(workerThread, "slotWriteToLora", Qt::QueuedConnection,
                                  Q_ARG(QString, message));
    }
}
void MainWindow::Send_Gas_Value() {
    ...
    if (currentSelectedNode >= 0) {
        QString message = QString("%1M%2").arg(currentSelectedNode + 1).arg(gas);
        QMetaObject::invokeMethod(workerThread, "slotWriteToLora", Qt::QueuedConnection,
                                  Q_ARG(QString, message));
    }
}
