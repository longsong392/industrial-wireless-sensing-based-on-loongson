...
const unsigned long FK[4] = { 0xa3b1bac6,0x56aa3350,0x677d9197,0xb27022dc };
...
void SM4_Enc(const unsigned char* src, unsigned char* dst, size_t* len, unsigned char iv[16], const unsigned char key[16]) {
    ...
    i = 16 - (*len % 16);
    unsigned char pBuf[256] = {0}; // 静态分配，最大明文长度256
    ...
    *len += i;
    ...
    i = *len;
    while (i > 0) {
        for (n = 0; n < 16; n++) { dst[n] = srcBuf[n] ^ iv[n]; }
        OneRound(sk, dst, dst);
        memcpy(iv, dst, 16);
        srcBuf += 16;
        dst += 16;
...
void SM4_Dec(const unsigned char* src, unsigned char* dst, size_t* len, unsigned char iv[16], const unsigned char key[16]) {
    ...
    for (n = 0; n < 16; n++) { SWAP(sk[n], sk[31 - n]); }
    unsigned char* fst = dst;
    i = *len;
    while (i > 0) {
        memcpy(tmp, src, 16);
        OneRound(sk, src, dst);
        for (n = 0; n < 16; n++) { dst[n] = dst[n] ^ iv[n]; }
        memcpy(iv, tmp, 16);
        src += 16;
        dst += 16;
        i -= 16;
